...
export default function AdminDashboard() {
    ...
    const [topNuts, setTopNuts] = useState(topNutritionists);
    const [stats, setStats] = useState({ totalUsers: 1248, totalOrders: 312, totalPartners: 8, totalAppointments: 0, pendingAppointments: 0 });
    ...
    useEffect(() => {
        adminService.getDashboard().then(data => {
            ...
            if (data.recentAppointments?.length > 0) setAppointments(data.recentAppointments);
            if (data.topNutritionists?.length > 0) setTopNuts(data.topNutritionists);
    ...
    return (
        <div className="min-h-screen bg-[#F5F1E8] p-6 lg:p-10">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                {[
                    { label: "ผู้ใช้งานทั้งหมด", value: stats.totalUsers.toLocaleString(), sub: "ทั้งหมดในระบบ", icon: <Users className="w-6 h-6" />, color: "bg-[#ffd980]" },
                    { label: "การนัดหมาย", value: stats.totalAppointments.toLocaleString(), sub: `${stats.pendingAppointments} รอยืนยัน`, icon: <CalendarCheck className="w-6 h-6" />, color: "bg-[#ffe9a0]" },
                    { label: "ออเดอร์ทั้งหมด", value: stats.totalOrders.toLocaleString(), sub: `จาก ${stats.totalPartners} ร้านค้าพาร์ทเนอร์`, icon: <Utensils className="w-6 h-6" />, color: "bg-[#ffd980]" },
