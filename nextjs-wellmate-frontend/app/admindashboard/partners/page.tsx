...
export default function PartnersPage() {
    ...
    useEffect(() => {
        adminService.getPartners().then(data => {
            const mapped = data.map((p: any) => ({
                id: `P${p.foodPartnerId}`,
                name: p.partnerName,
                owner: "N/A",
                email: p.contactEmail || "-",
                phone: p.contactPhone || "-",
                address: p.addressLine1 || "ไม่ระบุ",
                status: p.isActive ? "active" : "suspended",
                menu: p._count?.menuItems || 0,
                orders: p._count?.partnerOrders || 0,
                revenue: 0,
                commission: p.commissionRate || 15,
                joined: new Date(p.joinedAt).toLocaleDateString('th-TH'),
                rating: 4.5,
                category: p.categories?.[0] || "อาหารทั่วไป",
