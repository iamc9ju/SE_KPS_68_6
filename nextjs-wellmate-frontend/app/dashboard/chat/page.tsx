...
const RecommendationCard = ({ items, onAddToCart }: { items: any[]; onAddToCart: (item: any) => void }) => {
    ...
    useEffect(() => {
        if (items && Array.isArray(items)) {
            setSelectedIds(items.map(i => i.menuItem.menuItemId));
        }
    }, [items]);
...
export default function ChatPage() {
    ...
    const { sendMessage, markAsRead, onNewMessage, onMessagesRead, onRecommendationsUpdated, isConnected } = useChatWebSocket(selectedRoomId);
    ...
    // Fetch rooms (extracted as callback for reuse)
    ...
    // Listen for recommendations update (Patient only)
    useEffect(() => {
        if (user?.role !== "patient") return;
        
        return onRecommendationsUpdated((data) => {
            console.log("⭐ Recommendations Updated Event Received:", data);
            // Refresh rooms to get new recommendedItems
            fetchRooms();
            
            // Notification
            Swal.fire({
                title: "คำแนะนำใหม่!",
                text: "นักโภชนาการมีคำแนะนำเมนูอาหารใหม่ให้กับคุณ",
                icon: "success",
                toast: true,
                position: "top-end",
    ...
    }, [onRecommendationsUpdated, user?.role, fetchRooms]);
    ...
    useEffect(() => {
        if (activeRoom) {
            console.log("📍 Active Room Updated:", {
                id: activeRoom.chatRoomId,
                role: user?.role,
                recommendationsCount: activeRoom.appointment?.recommendedItems?.length || 0,
                recommendedItems: activeRoom.appointment?.recommendedItems
            });
        }
    }, [activeRoom, user?.role]);
    ...
    // Group rooms by Active and History
    const filteredRooms = useMemo(() => {
        const now = new Date();
        const active: ChatRoom[] = [];
        const history: ChatRoom[] = [];

        rooms.forEach(room => {
            const end = new Date(room.appointment.endTime);
            if (now > end) {
                history.push(room);
            } else {
                active.push(room);
        ...
        // Sort by most recent message or start time
        const sorter = (a: ChatRoom, b: ChatRoom) => {
            const timeA = a.chatMessages && a.chatMessages.length > 0 
                ? new Date(a.chatMessages[0].createdAt).getTime() 
                : new Date(a.appointment.startTime).getTime();
            const timeB = b.chatMessages && b.chatMessages.length > 0 
                ? new Date(b.chatMessages[0].createdAt).getTime() 
                : new Date(b.appointment.startTime).getTime();
            return timeB - timeA;
        };

        return activeTab === 'active' ? active.sort(sorter) : history.sort(sorter);
    }, [rooms, activeTab]);
        ...
        <div className="flex h-screen w-full bg-white overflow-hidden text-[#434343] font-sans ml-64">
                ...
                <div className="px-5 py-6 border-b border-gray-100/80">
                    ...
                    <div className="flex bg-gray-100 p-1 rounded-xl mb-4">
                        <button 
                            ...
                            className={`flex-1 py-2 text-[12px] font-black rounded-lg transition-all ${activeTab === 'active' ? 'bg-white text-[#87D039] shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                            ห้องคุยปัจจุบัน
                        </button>
                        <button 
                            onClick={() => setActiveTab('history')}
                            className={`flex-1 py-2 text-[12px] font-black rounded-lg transition-all ${activeTab === 'history' ? 'bg-white text-[#87D039] shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                        >
                            ประวัติย้อนหลัง
                        </button>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto px-3 py-4 custom-scrollbar space-y-2">
                    {filteredRooms.length === 0 ? (
                        <div className="text-center py-10 px-4">
                            <p className="text-gray-400 text-sm font-medium">ไม่พบรายการห้องแชทในหมวดนี้</p>
                        </div>
                    ) : (
                        filteredRooms.map((room) => {
                            const isPatient = user?.role === "patient";
                            const other = isPatient ? room.appointment.nutritionist : room.appointment.patient;
                            const lastMsg = room.chatMessages && room.chatMessages.length > 0 ? room.chatMessages[0] : null;
                            const start = new Date(room.appointment.startTime);
                            const end = new Date(room.appointment.endTime);
                            const now = new Date();
                            ...
                            if (now >= start && now <= end) {
                                ...
                                statusColor = "bg-[#C5F285] text-[#5AAA1D] animate-pulse";
                            } else if (now > end) {
                            ...
                            return (
                                <div
                                    ...
                                    className={`relative p-4 flex items-start gap-3 rounded-2xl cursor-pointer transition-all ${selectedRoomId === room.chatRoomId ? "bg-[#f0fde4] shadow-sm" : "hover:bg-gray-50"}`}
                                >
                                    ...
                                    <div className="relative w-12 h-12 rounded-full overflow-hidden flex-shrink-0 shadow-sm bg-gray-200">
                                        ...
                                        <div className={`absolute bottom-0 right-0 w-3 h-3 border-2 border-white rounded-full ${now >= start && now <= end ? 'bg-[#87D039]' : 'bg-gray-300'}`} />
                                    </div>

                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-1">
                                            <div className="flex flex-col gap-0.5 min-w-0">
                                                <p className="font-black text-[13px] text-gray-800 tracking-tight leading-tight">
                                                    คำปรึกษา {format(start, "d MMM", { locale: th })}
                                                </p>
                                                <p className="text-[11px] text-gray-500 font-bold truncate">
                                                    {isPatient ? `${other.firstName} ${other.lastName}` : `${other.firstName} ${other.lastName}`}
                                                </p>
                                            </div>
                                            <span className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shrink-0 ${statusColor}`}>
                                                {statusLabel}
                                            </span>
                                        </div>
                                        
                                        <div className="flex items-center gap-2 mt-1">
                                            <p className="text-[12px] text-gray-400 truncate flex-1 leading-tight">
                                                {lastMsg ? lastMsg.text : "ยังไม่มีข้อความ"}
                                            </p>
                                            {isPatient && room.appointment?.recommendedItems && room.appointment.recommendedItems.length > 0 && (
                                                <div className="flex items-center gap-1 bg-[#87D039]/10 px-1.5 py-0.5 rounded-md shrink-0">
                                                    <Smile size={10} className="text-[#87D039]" />
                                                    <span className="text-[9px] font-black text-[#5AAA1D]">{room.appointment.recommendedItems.length}</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
            ...
            <div className="flex-1 flex flex-col bg-[#F8F9FA] min-w-0 relative">
                <div className="flex items-center justify-between px-6 py-4 bg-white border-b border-gray-100 shadow-sm z-10">
                    ...
                    <div className="flex items-center gap-2">
                        {user?.role === "patient" && activeRoom?.appointment?.recommendedItems && activeRoom.appointment.recommendedItems.length > 0 && (
                            <button 
                                onClick={() => {
                                    const summaryEl = document.getElementById('recommendations-section');
                                    summaryEl?.scrollIntoView({ behavior: 'smooth' });
                                }}
                                className="flex items-center gap-2 px-4 py-1.5 bg-[#87D039] hover:bg-[#76b831] text-white rounded-full text-[11px] font-black shadow-md shadow-[#87D039]/20 transition-all active:scale-95"
                            >
                                <Smile size={14} />
                                <span>คำแนะนำโภชนาการ</span>
                                <span className="ml-1 bg-white/20 px-1.5 py-0.5 rounded-md">{activeRoom.appointment.recommendedItems.length}</span>
                            </button>
                        )}
                ...
                <div className="flex-1 overflow-y-auto px-6 py-6 space-y-4 custom-scrollbar">
                    ...
                    {user?.role === "patient" && activeRoom?.appointment?.recommendedItems && activeRoom.appointment.recommendedItems.length > 0 && (
                        <div id="recommendations-section" className="flex justify-start scroll-mt-24">
                            <RecommendationCard 
