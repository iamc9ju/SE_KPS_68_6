...
async function main() {
  ...
  try {
    const nutritionists = await prisma.nutritionist.findMany({
      where: { firstName: { startsWith: 'ธนัญ' } },
      include: { user: true, appointments: true }
    });

    console.log(`Found ${nutritionists.length} nutritionists starting with ธนัญ`);
    nutritionists.forEach(n => {
      console.log(`- ID: ${n.nutritionistId}, Name: ${n.firstName} ${n.lastName}, Email: ${n.user?.email}`);
      console.log(`  Appointments count: ${n.appointments.length}`);
    });
